...
void TestCli::testAdd()
{
    ...
    execCmd(addCmd,
            {"add",
             ...
             m_dbFile->fileName(),
             "/newuser-entry"});
    QVERIFY(m_stderr->readAll().contains("Enter password to unlock"));
...
void TestCli::testHelp()
{
    ...
    auto output = m_stdout->readAll();
    QVERIFY(output.contains("Available commands"));

    List listCmd;
    execCmd(helpCmd, {"help", "ls"});
    output = m_stdout->readAll();
    QVERIFY(output.contains(listCmd.description.toLatin1()));
}
